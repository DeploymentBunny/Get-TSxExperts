...
function set-TSxLogPath {
    ...
    if ($LogFolderPath) {
        ...
        if($(Get-Variable -Name "TSx*" | Where-Object {$_.Value -like "*$($LogFile)"})){
            ...
            Write-Warning -Message "You specified a log that doesn't exist, or hasn't been declared - all commands will be logged to: $($LogFile)"
            $Global:TSxCurrentLogFile = $LogFile
        }
    }
